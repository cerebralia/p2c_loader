...
/*
 ...
 - 2017/05/26 (1.50) - Removed ImFontConfig::MergeGlyphCenterV in favor of a more Sequence ImFontConfig::GlyphOffset.
...
static void BeginTooltipEx(bool override_previous_tooltip)
{
    ...
    ImGui::Begin(window_name, NULL, ImGuiWindowFlags_Tooltip|ImGuiWindowFlags_NoTitleBar|ImGuiWindowFlags_NoMove|ImGuiWindowFlags_NoResize|ImGuiWindowFlags_NoSavedSettings|ImGuiWindowFlags_AlwaysAutoResize,false);
}
...
static bool BeginPopupEx(const char* str_id, ImGuiWindowFlags extra_flags)
{
    ...
    bool is_open = ImGui::Begin(name, NULL, flags,false);
...
bool ImGui::BeginPopupModal(const char* name, bool* p_open, ImGuiWindowFlags extra_flags)
{
    ...
    bool is_open = ImGui::Begin(name, p_open, flags , false);
...
static bool BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
    ...
    bool ret = ImGui::Begin(title, NULL, size, -1.0f, flags,false );

	const ImVec2 value_bufsize = ImGui::CalcTextSize(name);

	if (extra_flags == SkeetChild)
	{
		...
		window->DrawList->AddLine(ImVec2(child_bb.x, child_bb.y), ImVec2(child_bb.x, child_bb.y + size_arg.y), ImColor(18, 18, 18));
		window->DrawList->AddLine(ImVec2(child_bb.x + size_arg.x, child_bb.y), ImVec2(child_bb.x + size_arg.x, child_bb.y + size_arg.y), ImColor(18, 18, 18));
		window->DrawList->AddLine(ImVec2(child_bb.x, child_bb.y + size_arg.y), ImVec2(child_bb.x + size_arg.x, child_bb.y + size_arg.y), ImColor(18, 18, 18));
		window->DrawList->AddLine(ImVec2(child_bb.x, child_bb.y), ImVec2(child_bb.x + 10, child_bb.y), ImColor(18, 18, 18));
		window->DrawList->AddLine(ImVec2(child_bb.x + 20 + value_bufsize.x, child_bb.y), ImVec2(child_bb.x + size_arg.x, child_bb.y), ImColor(18, 18, 18));
		window->DrawList->AddText(ImVec2(child_bb.x + 15, child_bb.y - 6), ImColor(150, 150, 150), name);
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags, bool skeet)
{
    return ImGui::Begin(name, p_open, ImVec2(0.f, 0.f), -1.0f, flags, skeet);
}

bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags, bool skeet)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    ...
    if (first_begin_of_the_frame)
    {
        ...
        else
        {
			...
			{
				if (skeet)
				window->DrawList->AddRectFilled(window->Pos - ImVec2(10, 10), window->Pos + ImVec2(window->Size.x + 10, window->Size.y + 10), ImColor(0.22f, 0.22f, 0.22f, 1.00f), window_rounding);
            ...
            if (!(flags & ImGuiWindowFlags_NoTitleBar))
                window->DrawList->AddRectFilled(title_bar_rect.GetTL(), title_bar_rect.GetBR(), GetColorU32((g.FocusedWindow && window->RootNonPopupWindow == g.FocusedWindow->RootNonPopupWindow) ? ImGuiCol_TitleBgActive : ImGuiCol_TitleBg), 0.f, ImGuiCorner_TopLeft|ImGuiCorner_TopRight);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (flags & ImGuiSelectableFlags_Disabled) PushStyleColor(ImGuiCol_Text, g.Style.Colors[ImGuiCol_TextDisabled]);
	{
		...
		if (selected)
			RenderTextClipped2(bb.Min, bb_with_spacing.Max, label, NULL, &label_size, ImVec2(0.0f, 0.0f));
	}
...
bool ImGui::BeginMainMenuBar()
{
    ...
    if (!Begin("##MainMenuBar", NULL, ImGuiWindowFlags_NoTitleBar|ImGuiWindowFlags_NoResize|ImGuiWindowFlags_NoMove|ImGuiWindowFlags_NoScrollbar|ImGuiWindowFlags_NoSavedSettings|ImGuiWindowFlags_MenuBar,false)
        || !BeginMenuBar())
    {
        End();
        PopStyleVar(2);
